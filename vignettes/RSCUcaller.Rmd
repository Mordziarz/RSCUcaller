...
fasta_path <- system.file("extdata/mitogenome_sequence.txt", package = "RSCUcaller")
RSCUcaller::prepare_fasta(samples_table = samples_table,path = fasta_path,file_out = "prepared_fasta.fasta")
```
...
get_RSCU_out <- get_RSCU(merged_sequences = prepared_fasta)
```
...
```{r}
get_RSCU_other(merged_sequences = prepared_fasta ,codon_table_id = 2,pseudo_count = 0)
```
...
get_RSCU_other2(merged_sequences = prepared_fasta,samples_table=samples_table,pseudo_count = 0)
```
...
```{r}
correlation(get_RSCU_out = get_RSCU_out, Species_x = "Apopellia_endiviifolia_A1", Species_y = "Apopellia_endiviifolia_B3", xlab = "title of x lab", ylab = "title of y lab")
```
...
```{r}
grouping_table <- system.file("extdata/grouping_table.csv", package = "RSCUcaller")
grouping_table <- read.csv2(grouping_table,sep = ";")

boxplot_between_groups_out <- boxplot_between_groups(get_RSCU_out = get_RSCU_out, grouping_table = grouping_table, width = 6, height = 6, xlab = "title of x lab", res = 300,p.adjust.method = "bonferroni")

head(boxplot_between_groups_out)
```
...
```{r}
stat_scat_box_out <- stat_scat_box(get_RSCU_out = get_RSCU_out, width = 6, height = 6, res = 300,p.adjust.method = "bonferroni")

head(stat_scat_box_out)
```
